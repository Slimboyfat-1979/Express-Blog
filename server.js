...
app.get("/", async (req, res) => {
  try {
    ...
    const filtered = response.data.filter(post => post);
    res.render("index.ejs", { posts: filtered });
  } catch (error) {
...
app.get("/:id", async function(req, res){
    try {
        ...
        res.status(200).json({message: "Error fetching posts"});
...
app.post("/api/posts", async (req, res) => {   
    try {
        ...
        res.status(500).json({message: "Error posting a new post"});
    }
});
...
// app.get("/new", (req, res) => {
// //   res.render("modify.ejs", { heading: "New Post", submit: "Create Post" });
// res.send("<h1>Render form here</h1>")
// });
